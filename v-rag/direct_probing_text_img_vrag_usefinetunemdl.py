...
def main_directprobing():
    ...
    model = model.to('cuda')
    lora_path = "path_to/finetuned RadFM/mdl_fine/"
    model_to_merge = PeftModel.from_pretrained(model, lora_path)
    merged_model = model_to_merge.merge_and_unload()
    print("merge original model and lora adapter together")
    ...
    for patient in tqdm(struct): 
            ...
            if len(images) == 1:
                ...
                for entity, ent_type in zip(ents, ent_types):
                    ...
                    if top_k == 0:
                        ...
                        question += [" Answer the question with only the word yes or no. Do not provide explanations. According to last query image and the reference images and reports, does the patient have " + problem + "? " ]
                    ...
                    with torch.no_grad():   
                        ...
                        generation = merged_model.generate(lang_x,vision_x)
        ...
        if save_count > 0 and save_count % 50 == 0:       
            ...
            with open(output_path + 'test-ncbi' + '-radfm_prob_vRAG_addimages_withreport_finetune_' + lora_path.split('/')[-1] + '_top' + str(top_k), "w") as outfile:
